...
class TestUserController < Minitest::Test
  ...
  def setup
    ...
    @adm = User.create(login: 'testadmin', name: 'Test Administrator', password: @pwd, email: 'admin@example.org', super: true, group: @adm_grp)

    @user = User.new(login: 'testuser', name: 'Test User', password: @pwd, email: 'test@example.org', group: @group)
    @mgr = User.new(login: 'testmgr', name: 'Test Manager', password: @pwd, email: 'mgr@example.org', group: @group)
    
    @group.users << @user
    @group.users << @mgr
    @group.save
    
    @group2 = Group.create(id: 'UCSD', name: 'group2')
...
# -----------------------------------------------------------------------------------------------
  def test_get_forgot_password
    ...
    assert last_response.body.include?(PidApp::HTML_CONFIG['header_recovery']), 'Did not get to the administration page!'
...
# -----------------------------------------------------------------------------------------------
  def test_get_reset_password
    ...
    assert last_response.body.include?(PidApp::HTML_CONFIG['header_recovery']), 'Did not get to the reset password page!'
...
# -----------------------------------------------------------------------------------------------
  def test_post_forgot_password
    post '/user/forgot', { reset: true, email: 'Noone' }
    assert last_response.body.include?(PidApp::MESSAGE_CONFIG['bad_reset_email']), "Bad email got through not flagged"

    post '/user/forgot', { reset: true, email: 'email@gmail' }
    assert last_response.body.include?(PidApp::MESSAGE_CONFIG['bad_reset_email']), "Bad email got through not flagged"

    post '/user/forgot', { reset: true, email: 'some@gmail.com' }
    puts last_response.body
    assert last_response.body.include?(PidApp::MESSAGE_CONFIG['user_password_reset_email']), "Valid email did should get through (but not really send an email)"
...
# -----------------------------------------------------------------------------------------------
  def test_post_register
    ...
    assert_equal 2, Maintainer.where(user: @mgr).count, "User is not a maintainer of 2 groups!"
    ...
    usr = User.find_by(login: 'blahblah')
