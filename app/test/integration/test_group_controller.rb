...
class TestGroupController < Minitest::Test
  ...
  def setup
    ...
    @adm = User.new(login: 'testadmin', name: 'Test Administrator', password: @pwd, email: 'admin@example.org', super: true, group: @adm_grp)
    @user = User.new(login: 'testuser', name: 'Test User', password: @pwd, email: 'test@example.org')
    @user2 = User.new(login: 'testseconduser', name: 'Test User 2', password: @pwd, email: 'test2@example.org')
    ...
    maintainer = Maintainer.new(group: @group, user: @mgr)
    maintainer.save
  end
...
# --------------------------------------------------------------------------------------------------------------
  def test_put_group
    ...
    assert last_response.ok?, "Did not receive a 200 status code, got a #{last_response.status}"
    assert last_response.body.include?(PidApp::HTML_CONFIG['header_group_view']), 'Did not get to the group page!'
    assert_equal 'Updated Name 1', Group.find_by(id: @group.id).name, 'The changes were not saved!'
    post '/user/logout'
    ...
    assert last_response.ok?, "Did not receive a 200 status code, got a #{last_response.status}"
    assert last_response.body.include?(PidApp::HTML_CONFIG['header_group_view']), 'Did not get to the group page!'
    assert_equal 'Updated Name 2', Group.find_by(id: @group.id).name, 'The changes were not saved!'
  end
...
# --------------------------------------------------------------------------------------------------------------
  def test_post_group
    ...
    assert last_response.ok?, "Did not receive a 200 status code, got a #{last_response.status}"
    assert Group.find_by(id: 'TEST').present?, "The changes were not saved! #{last_response.body}"
    assert last_response.body.include?(PidApp::HTML_CONFIG['header_group_view']), 'Did not get to the group page!'
...
# # --------------------------------------------------------------------------------------------------------------
  def test_delete_group
    ...
    group = Group.new(id: 'TEST', name: 'Test Group', description: 'testing')
    group.save

    delete '/group/TEST'
    assert last_response.ok?, 'Unable to delete the group!'
    assert_nil Group.find_by(id: 'TEST'), 'The group was not deleted!'
  end
...
# --------------------------------------------------------------------------------------------------------------
  def test_remove_maintainer_from_group
    ...
    maintainer = Maintainer.new(group: @group, user: @adm)
    maintainer.save
